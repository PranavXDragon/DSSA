...
def generate_vision_video():
    ...
    cache_dir = "scripts/.photo_cache_vision"
    ...
    json_path = "public/assets/data/our-vision_data.json"
    if not os.path.exists(json_path):
        json_path = "dist/assets/data/our-vision_data.json"
    ...
    for idx, item in enumerate(vision_data):
    ...
    print(f"Generating {len(vision_data)} The Beginning slides...")
    slides_np = []
    ...
    for i, item in enumerate(vision_data):
        ...
        draw.text((80, 80), "OUR VISION · DSSA", fill=(255, 255, 255), font=font_div)
        draw.text((width - 300, 80), f"SCENE {i+1:02d} OF {len(vision_data):02d}", fill=item["color"], font=font_div)

        slides_np.append(np.array(base))
        if i == 0:
            base.save("public/media/our-vision-slideshow-thumbnail.jpg", "JPEG", quality=95)
            base.save("dist/media/our-vision-slideshow-thumbnail.jpg", "JPEG", quality=95)

    import time
    ts = int(time.time())
    out_tmp = f"public/media/tmp_vision_{ts}.mp4"
    out_public = f"public/media/our-vision-slideshow-{ts}.mp4"
    out_dist = f"dist/media/our-vision-slideshow-{ts}.mp4"
    ...
    try:
        ...
        for slide_idx in range(len(vision_data)):
            curr_slide = slides_np[slide_idx]
            next_slide = slides_np[(slide_idx + 1) % len(vision_data)]
            ...
            for fpath in ['public/assets/data/cms_projects.json', 'dist/assets/data/cms_projects.json']:
                if os.path.exists(fpath):
                    with open(fpath, 'r', encoding='utf-8') as f:
                        pdata = json.load(f)
                    fl = next((p for p in pdata if p.get('slug') == 'our-vision'), None)
                    if fl and 'video' in fl:
                        fl['video']['url'] = f'/media/our-vision-slideshow-{ts}.mp4?v={ts}'
                        fl['video']['thumbnail'] = f'/media/our-vision-slideshow-thumbnail.jpg?v={ts}'
                        fl['image'] = f'/media/our-vision-slideshow-thumbnail.jpg?v={ts}'
                        fl['video']['filename'] = f'our-vision-slideshow-{ts}.mp4'
