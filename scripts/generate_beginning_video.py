...
def generate_beginning_video():
    ...
    cache_dir = "scripts/.photo_cache_beginning"
    ...
    json_path = "public/assets/data/beginning_events.json"
    if not os.path.exists(json_path):
        json_path = "dist/assets/data/beginning_events.json"
    ...
    for idx, item in enumerate(beginning_data):
    ...
    print(f"Generating {len(beginning_data)} The Beginning slides...")
    slides_np = []
    
    for i, item in enumerate(beginning_data):
        ...
        draw.text((80, 80), "ORIGIN STORY · DSSA", fill=(255, 255, 255), font=font_div)
        draw.text((width - 300, 80), f"SCENE {i+1:02d} OF {len(beginning_data):02d}", fill=item["color"], font=font_div)

        slides_np.append(np.array(base))
        if i == 0:
            base.save("public/media/beginning-slideshow-thumbnail.jpg", "JPEG", quality=95)
            base.save("dist/media/beginning-slideshow-thumbnail.jpg", "JPEG", quality=95)

    import time
    ts = int(time.time())
    out_tmp = f"public/media/tmp_begin_{ts}.mp4"
    out_public = f"public/media/beginning-slideshow-{ts}.mp4"
    out_dist = f"dist/media/beginning-slideshow-{ts}.mp4"
    ...
    try:
        ...
        for slide_idx in range(len(beginning_data)):
            curr_slide = slides_np[slide_idx]
            next_slide = slides_np[(slide_idx + 1) % len(beginning_data)]
            ...
            for fpath in ['public/assets/data/cms_projects.json', 'dist/assets/data/cms_projects.json']:
                if os.path.exists(fpath):
                    with open(fpath, 'r', encoding='utf-8') as f:
                        pdata = json.load(f)
                    fl = next((p for p in pdata if p.get('slug') == 'the-beginning'), None)
                    if fl and 'video' in fl:
                        fl['video']['url'] = f'/media/beginning-slideshow-{ts}.mp4?v={ts}'
                        fl['video']['thumbnail'] = f'/media/beginning-slideshow-thumbnail.jpg?v={ts}'
                        fl['image'] = f'/media/beginning-slideshow-thumbnail.jpg?v={ts}'
                        fl['video']['filename'] = f'beginning-slideshow-{ts}.mp4'
